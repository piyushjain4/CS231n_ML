...
class ThreeLayerConvNet(object):
    ...
    def loss(self, X, y=None):
        ...
        ############################################################################
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        loss = soft_loss+0.5*self.reg*(np.sum(W1**2)+np.sum(W2**2)+np.sum(W3**2))
        dx_aff,dw3,db3 = affine_backward(soft_dx,cache)
        dx_aff_relu,dw2,db2 = affine_relu_backward(dx_aff,aff_cache)
        dx,dw1,db1 = conv_relu_pool_backward(dx_aff_relu,conv_cache)
        pass

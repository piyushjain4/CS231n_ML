...
def make_fooling_image(X, target_y, model):
    ...
    while scores.argmax(1) != target_y:
      scores = model(X_fooling)
      s = scores[:,target_y]
      s.backward()
      g = X_fooling.grad
      with torch.no_grad():
        X_fooling += (learning_rate*g)/((torch.sum(torch.square(g)))+1e-3)
        X_fooling.grad.zero_()
      _, preds = scores.max(1)
      if i%10 ==0 :
        print(f'Iteration {i},predicted_label = {preds}, actual_label = {target_y}')
      i+=1
...
def class_visualization_update_step(img, model, target_y, l2_reg, learning_rate):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    scores = model(img)
    s = scores[:,target_y].sum()
    (s-l2_reg*torch.sum(torch.square(img))).backward()
    with torch.no_grad():
